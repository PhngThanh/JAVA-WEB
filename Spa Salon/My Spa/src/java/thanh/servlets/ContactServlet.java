...
import thanh.daos.ContactDAO;
import thanh.daos.TimeDAO;
...
public class ContactServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            ContactDTO info = dao.getContactInfo();
            TimeDAO timeDao = new TimeDAO();
            List<TimeDTO> timeList = timeDao.getAllTime();
            request.setAttribute("CONTACTINFO", info);
            request.setAttribute("TIME", timeList);
            url = CONTACT_PAGE;
        } catch (SQLException e) {
            
            log("Contact_SQL" + e.getMessage());
        } catch (NamingException e) {
            log("Contact_Naming" + e.getMessage());
        } finally {
